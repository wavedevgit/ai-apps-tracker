(() => {
    "use strict";
    var e = {};
    const t = require("node:worker_threads"),
        n = [{
            name: "WebGL/GPU",
    ...
    function r(e) {
        ...
        if (e.samples && e.timeDeltas)
            for (let s = 0; s < e.samples.length; s++) {
                const i = e.samples[s],
                    o = e.timeDeltas[s] || 0;
                a.set(i, (a.get(i) || 0) + 1), c.set(i, (c.get(i) || 0) + o);
                const r = n.get(i);
                if (void 0 !== r) {
                    const e = t.get(r),
                        n = t.get(i);
        ...
        for (let e = 0; e < Math.min(20, y.length); e++) {
            const t = y[e],
                n = t.selfHits * F,
                i = (t.selfHits / m * 100).toFixed(2),
                r = t.url ? `${s(t.url)}:${t.lineNumber}` : "[native]";
            $.push(`| ${e+1} | \`${t.functionName}\` | ${r} | ${o(n)} | ${i}% |`)
        }
        const k = [];
        for (const [, e] of d)
            for (const t of e.positionTicks) k.push({
                functionName: e.functionName,
                url: e.url,
                line: t.line,
                ticks: t.ticks
            });
        if (k.sort((e, t) => t.ticks - e.ticks), k.length > 0) {
            $.push("\n## Hot Lines\n"), $.push("Exact source lines where CPU time is spent:\n"), $.push("| Function | Location | Line | Ticks | % |"), $.push("|----------|----------|------|-------|---|");
            for (const e of k.slice(0, 15)) {
                const t = (e.ticks / m * 100).toFixed(2),
                    n = e.url ? s(e.url) : "[native]";
                $.push(`| \`${e.functionName}\` | ${n} | ${e.line} | ${e.ticks} | ${t}% |`)
            }
        }
        const T = Array.from(f.values()).filter(e => "(root)" !== e.callerName && "(idle)" !== e.callerName && "(idle)" !== e.calleeName).sort((e, t) => t.hits - e.hits);
        if (T.length > 0) {
            $.push("\n## Hot Call Paths\n"), $.push("Which function calls are most expensive:\n"), $.push("| Caller | Callee | Calls | % |"), $.push("|--------|--------|-------|---|");
            for (const e of T.slice(0, 15)) {
                const t = (e.hits / m * 100).toFixed(2);
        ...
        const b = [],
            x = g.get("GC");
        if (x && h > 0 && x.hits / h > .05) {
            const e = (x.hits / h * 100).toFixed(1);
            b.push(`**GC**: ${e}% of active time - high allocation pressure, consider object pooling or reducing allocations`)
        }
        const w = g.get("WebGL/GPU");
        if (w && h > 0 && w.hits / h > .1) {
            const e = y.find(e => "WebGL/GPU" === i(e.functionName));
            e && b.push(`**WebGL**: \`${e.functionName}\` taking ${o(e.selfHits*F)} - consider batching draw calls or reducing texture uploads`)
        }
        const v = [];
        for (const [, e] of d) e.totalHits > 5 * e.selfHits && e.selfHits > .01 * h && v.push({
            name: e.functionName,
            hits: e.totalHits
        });
        v.sort((e, t) => t.hits - e.hits);
        for (const e of v.slice(0, 3)) {
            const t = e.hits * F;
            b.push(`**Deep call stack**: \`${e.name}\` - ${o(t)} total time with deep recursion/call chains`)
        }
        if (y.length > 0 && h > 0) {
            const e = y[0];
            if (e.selfHits > .2 * h) {
                const t = (e.selfHits / h * 100).toFixed(1);
                b.push(`**Single bottleneck**: \`${e.functionName}\` consuming ${t}% of active CPU time`)
            }
        }
        if (b.length > 0) {
            $.push("\n## Warnings\n");
            for (const e of b) $.push(`- ${e}`)
        }
        if ($.push("\n## Call Tree\n"), $.push("```"), u && function e(n, i, o, r) {
                const c = t.get(n);
                if (!c) return;
                const f = a.get(n) || 0,
                    u = l.get(n) || 0;
                ...
                const {
                    ...
                    url: h,
                    lineNumber: d
                } = c.callFrame, g = p || "(anonymous)", N = m > 0 ? (u / m * 100).toFixed(1) : "0", y = m > 0 ? (f / m * 100).toFixed(1) : "0", F = h ? ` ${s(h)}:${d}` : "", k = 0 === i ? "" : r ? "└─ " : "├─ ", T = f > 0 ? ` [self: ${y}%]` : "";
                if ($.push(`${o}${k}${g} (${N}%${T})${F}`), c.children && c.children.length > 0) {
                    const t = c.children.map(e => ({
                            id: e,
                            hits: l.get(e) || 0
                        })).filter(e => e.hits > 0 && e.hits / m > .005).sort((e, t) => t.hits - e.hits).slice(0, 10),
                        n = o + (0 === i ? "" : r ? "   " : "│  ");
                    for (let s = 0; s < t.length; s++) e(t[s].id, i + 1, n, s === t.length - 1)
                }
...
//# sourceMappingURL=http://go/sourcemap/sourcemaps/93e603f703cd553a6bb3644711a3379bbbb31180/extensions/cursor-browser-automation/dist/cpuProfileWorker.js.map