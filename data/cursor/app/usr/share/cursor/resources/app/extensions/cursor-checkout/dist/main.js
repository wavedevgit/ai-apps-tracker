(() => {
    ...
    class c {
        ...
        async checkoutBranch(e, t, a, r) {
            ...
            try {
                if (this.throwIfCancelled(r), "" !== (await this.executeGitCommandStable(i, ["status", "--porcelain"], n.signal)).trim()) {
                    const a = await e.resolveConflict();
                    if (this.throwIfCancelled(r), "stash" === a) await this.executeGitCommandStable(i, ["stash", "--include-untracked"], n.signal);
                    else if ("commit" === a) await this.executeGitCommandStable(i, ["add", "-A"], n.signal), await this.executeGitCommandStable(i, ["commit", "-m", `checkpoint before checking out ${t}`], n.signal);
                    else {
                        if ("discard" !== a) throw new Error("Aborting checkout");
                        await this.executeGitCommandStable(i, ["reset", "--hard", "HEAD", "--quiet"], n.signal), await this.executeGitCommandStable(i, ["clean", "-fd", "--quiet"], n.signal)
                    }
                }
                if (a?.localOnly) {
                    const e = `refs/heads/${s}`;
                    return await this.executeGitCommandStable(i, ["rev-parse", "--verify", e], n.signal), void await this.executeGitCommandStable(i, ["checkout", s], n.signal)
                }
                this.wasBranchWarmedRecently(s) || (await this.executeGitCommandStable(i, ["fetch", "origin", `refs/heads/${s}:refs/remotes/origin/${s}`, "--no-tags", "--recurse-submodules=no", "--no-write-fetch-head"], n.signal), this.setBranchWarmTimestamp(s));
                let o = !1;
                const c = `refs/heads/${s}`,
                    l = `refs/remotes/origin/${s}`;
                try {
                    await this.executeGitCommandStable(i, ["rev-parse", "--verify", c], n.signal), o = !0
                } catch {
                    o = !1
                }
                if (o) await this.executeGitCommandStable(i, ["checkout", s], n.signal);
                else {
                    let e = !1;
                    try {
                        await this.executeGitCommandStable(i, ["rev-parse", "--verify", l], n.signal), e = !0
                    } catch {
                        e = !1
                    }
                    if (!e) throw new Error(`Remote branch not found on origin: ${s}`);
                    await this.executeGitCommandStable(i, ["checkout", "-b", s, `origin/${s}`], n.signal)
                }
                await this.executeGitCommandStable(i, ["merge", "--ff-only", `origin/${s}`], n.signal)
            } finally {
                o?.dispose()
            }
        }
        async ensureCurrentBranchPushed(e, t) {
            ...
            try {
                this.throwIfCancelled(t);
                const s = await this.executeGitCommandStable(a, ["rev-parse", "--abbrev-ref", "HEAD"], i.signal),
                    n = this.normalizeCurrentBranchRef(s),
                    o = this.resolveCurrentBranchName(n),
                    c = `refs/remotes/${r}/${o}`;
                let l = !1;
                try {
                    const e = await this.executeGitCommandStable(a, ["rev-list", "--count", `${c}..HEAD`], i.signal);
                    l = Number(e.trim()) > 0
                } catch {
                    l = !0
                }
                if (!l) return;
                const h = ["push", r, `HEAD:refs/heads/${o}`];
                !0 === e?.forcePush && h.push("--force-with-lease"), await this.executeGitCommandStable(a, h, i.signal)
            } finally {
                s?.dispose()
            }
        }
...
//# sourceMappingURL=http://go/sourcemap/sourcemaps/87903b25fe9074e35b9ba372ed5bf14de5835960/extensions/cursor-checkout/dist/main.js.map