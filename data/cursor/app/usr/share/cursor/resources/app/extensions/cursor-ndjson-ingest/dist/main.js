(() => {
    ...
    var e = {
            ...
            184: function(e, t, r) {
                var o, n = this && this.__createBinding || (Object.create ? function(e, t, r, o) {
                    ...
                    i = this && this.__importStar || (o = function(e) {
                        ...
                        var t = {};
                        if (null != e)
                            for (var r = o(e), i = 0; i < r.length; i++) "default" !== r[i] && n(t, e, r[i]);
                        return s(t, e), t
                    });
                ...
                function P(e) {
                    return "classic" === e ? {
                        ...
                        allocatedPortKey: `ndjson.allocatedPort.${e}`,
                        ingestPathIdKey: `ndjson.targetId.${e}`
                    }
                }
            ...
            377: function(e, t, r) {
                ...
                Object.defineProperty(t, "__esModule", {
                    value: !0
                }), t.NdjsonIngestLogger = void 0;
                const a = i(r(398));
                class c {
                    static init() {
                        c.output = a.window.createOutputChannel("NDJSON Ingest", {
                            log: !0
                        })
                    }
                    static error(e, ...t) {
                        c.output || c.init(), c.output?.error(e, ...t)
                    }
                    static warn(e, ...t) {
                        c.output || c.init(), c.output?.warn(e, ...t)
                    }
                    static info(e, ...t) {
                        c.output || c.init(), c.output?.info(e, ...t)
                    }
                    static debug(e, ...t) {
                        c.output || c.init(), c.output?.debug(e, ...t)
                    }
                    static trace(e, ...t) {
                        c.output || c.init(), c.output?.trace(e, ...t)
                    }
                }
                t.NdjsonIngestLogger = c
            },
            398: e => {
            ...
            751: function(e, t, r) {
                ...
                Object.defineProperty(t, "__esModule", {
                    value: !0
                }), t.isPortAvailable = async function(e, t) {
                    return new Promise(r => {
                        const o = a.createServer();
                        o.once("error", () => r(!1)), o.once("listening", () => {
                            o.close(() => r(!0))
                        }), o.listen(e, t)
                    })
            ...
            882: function(e, t, r) {
                ...
                Object.defineProperty(t, "__esModule", {
                    value: !0
                }), t.NdjsonIngestServer = t.NdjsonServerStartError = t.DEFAULT_NDJSON_PORT_RANGE_END = t.DEFAULT_NDJSON_PORT_RANGE_START = t.isPortAvailable = t.resolveLogger = void 0, t.chooseNdjsonPort = async function(e) {
                    const r = e.configuredPort ?? 0;
                    if (0 !== r) return r;
                    const o = new Set((e.excludedPorts ?? []).filter(e => e > 0)),
                        n = e.previouslyAllocatedPort ?? 0;
                    if (0 !== n && !o.has(n)) {
                        if (await (0, f.isPortAvailable)(n, e.bindAddress)) return (0, l.resolveLogger)(e.logger).info(`Reusing previously allocated port ${n}`), n;
                        (0, l.resolveLogger)(e.logger).info(`Previously allocated port ${n} is unavailable; scanning for a new port`)
                    }
                    const s = e.rangeStart ?? t.DEFAULT_NDJSON_PORT_RANGE_START,
                        i = e.rangeEnd ?? t.DEFAULT_NDJSON_PORT_RANGE_END;
                    if (s > i) throw new Error(`Invalid port range ${s}-${i}`);
                    const a = i - s + 1,
                        c = Math.floor(Math.random() * a);
                    for (let t = 0; t < a; t++) {
                        const r = s + (c + t) % a;
                        if (!o.has(r) && await (0, f.isPortAvailable)(r, e.bindAddress)) return await (e.onAutoAllocatedPort?.(r)), (0, l.resolveLogger)(e.logger).info(`Auto-allocated port ${r} and saved to internal storage`), r
                    }
                    throw new Error(`No available ports in range ${s}-${i}`)
                };
                const a = r(474),
                    c = i(r(24)),
                    u = i(r(67)),
                    ...
                    f = r(751);
                var p = r(908);
                Object.defineProperty(t, "resolveLogger", {
                    enumerable: !0,
                    get: function() {
                        return p.resolveLogger
                    }
                });
                var h = r(751);
                Object.defineProperty(t, "isPortAvailable", {
                    enumerable: !0,
                    get: function() {
                ...
                t.NdjsonServerStartError = v, t.NdjsonIngestServer = class {
                    ...
                    getLoopbackUrl() {
                        if (this.currentPort) return `http://${this.options.bindAddress}:${this.currentPort}/ingest/${this.options.ingestPathId}`
                    }
                    getLogDirectory() {
                    ...
                    async start() {
                        ...
                        return this.currentPort = t?.port ?? 0, this.logger.info(`NDJSON ingest server started on port ${this.currentPort} in directory ${this.options.logDirectory}`), this.buildStartResult()
                    }
                    async stop() {
                        const e = this.server;
                        e && (e.closeAllConnections(), e.close(), await (0, a.once)(e, "close"), this.server = void 0, this.currentPort = 0)
                    }
                    async appendExclusive(e, t) {
                        const r = async () => {
                            const r = c.createWriteStream(t, {
                                flags: "a"
                            });
                            await (0, g.pipeline)(e, (0, f.ensureTrailingNewline)(), r)
                        }, o = this.writeQueue.then(r, r);
                        return this.writeQueue = o.catch(e => {
                            this.logger.error("Error appending to log file:", e)
                        }), o
                    }
                    buildStartResult() {
                        const e = this.getLoopbackUrl();
                        if (!e) throw new Error("NDJSON server is not listening");
                        return {
                            loopbackUrl: e,
                    ...
                    async handleRequest(e, t) {
                        if (t.setHeader("Access-Control-Allow-Origin", "*"), t.setHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS"), t.setHeader("Access-Control-Allow-Headers", "Content-Type, X-Debug-Session-Id"), "OPTIONS" === e.method) return t.writeHead(200), void t.end();
                        if ("POST" !== e.method || e.url !== `/ingest/${this.options.ingestPathId}`) return t.writeHead(404), void t.end();
                        const r = e.headers["x-debug-session-id"];
                        if (void 0 === r) return this.logger.warn("Rejected NDJSON ingest request without X-Debug-Session-Id header"), t.writeHead(400), void t.end("missing-session-id");
                        if ("string" != typeof r) return this.logger.warn("Rejected NDJSON ingest request with invalid X-Debug-Session-Id header"), t.writeHead(400), void t.end("invalid-session-id");
                        const o = (0, f.sanitizeSessionId)(r);
                        if (!o) return this.logger.warn("Rejected NDJSON ingest request with invalid X-Debug-Session-Id header"), t.writeHead(400), void t.end("invalid-session-id");
                        try {
                            const r = this.getLogPathForSession(o);
                            await this.appendExclusive(e, r), t.writeHead(204), t.end()
                        } catch (e) {
                            this.logger.error("NDJSON ingest write error:", e), t.writeHead(500), t.end("write-failed")
                        }
                    }
                    async listen(e) {
                        e.listen(this.options.port, this.options.bindAddress), await (0, a.once)(e, "listening")
                    }
                }
            },
            908: (e, t) => {
...
//# sourceMappingURL=http://go/sourcemap/sourcemaps/a80ff7dfcaa45d7750f6e30be457261379c29b00/extensions/cursor-ndjson-ingest/dist/main.js.map