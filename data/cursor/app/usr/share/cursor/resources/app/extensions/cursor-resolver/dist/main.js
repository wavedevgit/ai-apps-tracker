(() => {
    ...
    class i extends Error {
        constructor(e, t = n.Unknown, r, o, i) {
            super(function(e, t) {
                return e.length ? `[${s(t)}] ${e}` : `[${s(t)}]`
            }(e, t)), this.name = "ConnectError", Object.setPrototypeOf(this, new.target.prototype), this.rawMessage = e, this.code = t, this.metadata = new Headers(null != r ? r : {}), this.details = null != o ? o : [], this.cause = i
        }
        static from(e, t = n.Unknown) {
            return e instanceof i ? e : e instanceof Error ? "AbortError" == e.name ? new i(e.message, n.Canceled) : new i(e.message, t, void 0, void 0, e) : new i(String(e), t, void 0, void 0, e)
        }
        static[Symbol.hasInstance](e) {
            return e instanceof Error && (Object.getPrototypeOf(e) === i.prototype || "ConnectError" === e.name && "code" in e && "number" == typeof e.code && "metadata" in e && "details" in e && Array.isArray(e.details) && "rawMessage" in e && "string" == typeof e.rawMessage && "cause" in e)
        }
        findDetails(e) {
    ...
    function u(e) {
        const t = function(e) {
            return e instanceof i || "object" == typeof e && null !== e && "code" in e && "number" == typeof e.code ? e.code : void 0
        }(e);
        return t === n.DeadlineExceeded || t === n.Unavailable
    }
    async function d(e) {
    ...
    function h(...e) {
        ...
        console.log(`[cursor-resolver]  ${t}`), p?.appendLine(`[INFO]  ${t}`)
    }

    function y(...e) {
        const t = e.map(String).join(" ");
        p?.appendLine(`[ERROR] ${t}`)
    }

    function v() {
        const e = r.cursor.productCommit ?? r.cursor.cursorServerCommit;
        if (!e || !/^[a-zA-Z0-9\-_.]+$/.test(e)) throw y("Invalid or missing product commit", e), new Error("Invalid or missing product commit");
        return e
    }
    class f {
        constructor(e, t) {
            this.connectionTokenProvider = e, this.alwaysShowPortsView = !0,
                function(e) {
                    p = e
                }(t), h("RemoteAuthorityResolver constructor")
        }
        ...
        async showCandidatePort(e, t, r) {
            ...
            }(t) || (h(`Filtering out reserved/internal port ${t} from candidates`), !1)
        }
        async getCursorServerUrl(e, t = !0) {
            ...
            const o = n,
                s = v();
            return await g("getCursorServerUrl", async () => {
                try {
                    return await this.connectionTokenProvider.getOrCreateCursorServerUrl(o, s, t)
                } catch (e) {
                    throw y("Error getting cursor server url", e), e
                }
            })
        }
        async getCursorServerUrlWithRetry(e, t = !0) {
            return async function({
                ...
                for (let t = 0;; t++) try {
                    ...
                    if (!u(e)) throw e;
                    const s = c(t);
                    n?.(e, t + 1, s), s > 0 && await r(s), o = !1
                }
            }({
                initialUseCache: t,
                getCursorServerUrl: t => this.getCursorServerUrl(e, t),
                onRetry: (e, t, r) => {
                    h("Error getting cursor server url,", r > 0 ? `retrying in ${r/1e3}s` : "retrying immediately", `attempt=${t}`, e)
                }
            })
        }
        async resolve(e, t, n) {
            return g("resolve", async () => {
                h("resolve", e, `resolveAttempt=${t.resolveAttempt}`), n?.report({
                    phase: "init"
                });
                const o = e.indexOf("+"),
                    s = e.substring(o + 1).trim();
                if (-1 === o || 0 === s.length) throw new Error("No bcId found in authority");
                if (s.startsWith("{")) {
                    let e;
                    try {
                        e = JSON.parse(s)
                    ...
                    h("resolved url (inline)", e.host, e.port);
                    const t = async () => {
                        try {
                            return await m(e)
                        } catch (e) {
                ...
                const a = v(),
                    {
                        connectionToken: c
                    } = await this.connectionTokenProvider.getOrCreateConnectionToken(i, a);
                n?.report({
                    phase: "get-url"
                });
                const u = this.getCursorServerUrlWithRetry(e, t.resolveAttempt < 3),
                    d = async () => {
                        ...
                        const e = await u;
                        h("resolved url", e.host, e.port), n?.report({
                            phase: "socket"
                        });
                        try {
                            return await m(e)
                ...
                return this.createManagedResolvedAuthority(d, c, p)
            })
        }
    }
    async function m(e) {
        ...
        h("tcp connection established", `${e.host}:${e.port}`, t ? "(tls)" : "(plain)");
    ...
    function w(e) {
        ...
        const o = new f(r.cursor.connectionTokenProvider, t);
...
//# sourceMappingURL=http://go/sourcemap/sourcemaps/3dc559280adc5f931ade8e25c7b85393842acf30/extensions/cursor-resolver/dist/main.js.map